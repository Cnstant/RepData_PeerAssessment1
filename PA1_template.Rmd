...
Averaged through the days, the interval with the maximum activity in terms of steps is `r hour[Total.interval$steps == max(Total.interval$steps)][1]`. It may corresponds to the time of departure to the user's workplace
```{r}
...
```
As a result `r 100*sum(is.na(data$steps))/dim(data)[1]`% of the variable "steps" is composed by NA values
